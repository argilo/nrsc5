...
** the file structure, and dump it back out as an HDC file.
**
** Authors: Andrew Wesie, Clayton Smith
**
**
**
...
static void parse_sbr_header(bitreader_t *br, bitwriter_t *bw)
{
    bw_addbits(bw, br_readbits(br, 1), 1);

    bw_addbits(bw, br_readbits(br, 4), 4);
    bw_addbits(bw, br_readbits(br, 4), 4);
    bw_addbits(bw, br_readbits(br, 3), 3);
    bw_addbits(bw, br_readbits(br, 2), 2);
    uint8_t extra1 = br_readbits(br, 1);
    bw_addbits(bw, extra1, 1);
    uint8_t extra2 = br_readbits(br, 1);
    bw_addbits(bw, extra2, 1);
...
static void parse_sbr_single_channel_element(bitreader_t *br, bitwriter_t *bw)
{
    uint8_t b = br_readbits(br, 1);
    bw_addbits(bw, b, 1);
    if (b)
    ...
    bw_addbits(bw, 0, 1); // FIXME HDC specific?
...
static void parse_sbr_channel_pair_element(bitreader_t *br, bitwriter_t *bw)
{
    uint8_t b = br_readbits(br, 1);
    bw_addbits(bw, b, 1);
    if (b)
...
static void parse_sbr(bitreader_t *br, bitwriter_t *bw, ics_t *ics, unsigned int count)
{
    bw_addbits(bw, ID_FIL, LEN_SE_ID);

    bw_addbits(bw, 1, 1); // SBR present
    uint8_t header_flag = br_readbits(br, 1);
    bw_addbits(bw, header_flag, 1);
    if (header_flag)
        parse_sbr_header(br, bw);

    if (ics->id == 0)
        parse_sbr_single_channel_element(br, bw);
    else
        parse_sbr_channel_pair_element(br, bw);
}

static void parse_fil(bitreader_t *br, bitwriter_t *bw, ics_t *ics)
{
    unsigned int count;
    if (br_readbits(br, 3) == 6)
    {
        count = br_readbits(br, 4);
        if (count == 15)
            count += (br_readbits(br, 8) - 1);
        if (br_readbits(br, 4) == EXT_SBR_DATA)
            parse_sbr(br, bw, ics, count);
    }
}
...
static void parse_ics_info(bitreader_t *br, bitwriter_t *bw, ics_t *ics)
{
    ...
    if (ics->window_sequence != EIGHT_SHORT_SEQUENCE)
    {
        br_readbits(br, 1); // predictor_data_present
    }
}

static void parse_tns_data(bitreader_t *br, bitwriter_t *bw, ics_t *ics)
{
    uint8_t w, filt, i, start_coef_bits, coef_bits, filter;
    ...
    for (w = 0; w < ics->num_windows; w++)
    {
        ...
        {
            ics->tns.n_filt[w] = br_readbits(br, 2);
        }
        ...
        for (filter = 0; filter < ics->tns.n_filt[w]; filter++)
        {
...
static void parse_side_info(bitreader_t *br, bitwriter_t *bw, ics_t *ics)
{
    ...
    global_gain = br_readbits(br, 8);
    bw_addbits(bw, global_gain, 8); // global gain
    ...
    br_readbits(br, 1); // pulse_data_present
    ics->tns.present = br_readbits(br, 1);
    if (ics->tns.present)
        parse_tns_data(br, bw, ics);
    br_readbits(br, 1); // gain_control_data_present
}
...
static void parse_sce(bitreader_t *br, bitwriter_t *bw)
{
    ...
    br_readbits(br, LEN_TAG);

    parse_ics_info(br, bw, &ics);

    bw_addbits(bw, 0, 1); // tns_data_present
    // TODO: TNS data
...
static void parse_cpe(bitreader_t *br, bitwriter_t *bw)
{
    ...
    br_readbits(br, LEN_TAG);
    br_readbits(br, 1); // common window
...
static void parse_packet(bitreader_t *br, bitwriter_t *bw)
{
    ...
    // Todo: read padding and ID_END?
}

void aac_to_hdc(bitreader_t *br, bitwriter_t *bw)
{
    parse_packet(br, bw);
}
